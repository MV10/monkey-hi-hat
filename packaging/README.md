...
This is a content-only .csproj (no build output or assembly).
...
* Windows: 
  ...
  * Update the versioned filename in `ReleaseConstants.cs`
* Linux:
...
* Zip the following files into `ndi-x-x-x.zip` (version on NDI Tools page)
  ...
  * `libndi.so`
* Upload .zip file to `monkeyhihat.com/public_html/installer_assets` via cpanel File Manager
* Update the versioned filenames in `ReleaseConstants.cs`, `install.sh`, and `update.sh`
...
### .NET
  * Windows: update version-check and download URL in `ReleaseConstants.cs`
  * Linux: 
