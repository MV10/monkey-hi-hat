...
// All config changes should be noted on the docs Changelog page
// https://www.monkeyhihat.com/docs/index.php#/changelog

// If versionFound is 1.0.0 that means version.txt was found but can't be parsed.
// v3.1.0 was the first to include a version.txt. Linux support was officially added
// in v5.2.0, so that by definition is always a new install, and v5.3.0 or later are
// the versions that could be Windows or Linux updates.
...
public static class ConfigHelper
{
    ...
    public static void NewWindowsInstall()
    {
        Output.Write($"Creating application config file...");

        Output.Write("-- Copying default config to app directory");
        var src = Path.Combine(ReleaseConstants.programPath, "ConfigFiles", "mhh.conf");
        File.Copy(src, confPathname);

        Output.Write("-- Adding references to viz content directories");

        var vizPath = Path.Combine(ReleaseConstants.contentPath, "shaders");
        var libPath = Path.Combine(ReleaseConstants.contentPath, "libraries");
        var fxPath = Path.Combine(ReleaseConstants.contentPath, "fx");
        var playlistPath = Path.Combine(ReleaseConstants.contentPath, "playlists");
        var texturePath = Path.Combine(ReleaseConstants.contentPath, "textures");
        var crossfadePath = Path.Combine(ReleaseConstants.contentPath, "crossfades");
        ...
        AddReplacement("windows", "#FFmpegPath=", "FFmpeg Path", $"FFmpegPath={ReleaseConstants.FFmpegPath}");
    ...
    public static void Update()
    {
        ...
        switch(Program.versionFound.ToString(3))
        {
            ...
            // ALL VERSIONS ABOVE ARE WINDOWS-ONLY
            // 5.2.0 was the first Linux release, so it was by definition a
            ...
            default:
                Output.Write($"-- No changes; installed version {Program.versionFound.ToString(3)} is not recognized.");
                return;
        }
